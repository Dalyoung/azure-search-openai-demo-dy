...
./scripts/.venv/bin/python ./scripts/prepdocs_novel.py './data/hcc_docs_novel/Japan*' --storageaccount "$AZURE_STORAGE_ACCOUNT" --container "$AZURE_STORAGE_CONTAINER" --searchservice "$AZURE_SEARCH_SERVICE" --index "hcc-poc-novel-index" --formrecognizerservice "$AZURE_FORMRECOGNIZER_SERVICE" --tenantid "$AZURE_TENANT_ID" -v
